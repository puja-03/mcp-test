...
export default function Edit({ exam, batches }: any) {
    const { data, setData, put, processing, errors } = useForm({ batch_id: exam.batch_id||'', title: exam.title||'', description: exam.description||'', exam_date: exam.exam_date||'', max_marks: exam.max_marks||'', passing_marks: exam.passing_marks||'' });
    return (<div className="p-6 max-w-2xl mx-auto"><Head title="Edit Exam" /><h1 className="text-2xl font-bold mb-6">Edit Exam</h1>
        <form onSubmit={e => { e.preventDefault(); put(`/tenant/exams/${exam.id}`); }} className="space-y-4">
