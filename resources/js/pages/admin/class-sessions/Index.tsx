...
export default function Index({ sessions, batches, filters }: any) {
    ...
    const handleDelete = (id: number) => {
        if (confirm('Are you sure you want to delete this class session? This will remove attendance records associated with it.')) {
            router.delete(`/admin/class-sessions/${id}`);
        }
    };

    return (
        <AppEliteCoachLayout title="Session Scheduler">
            ...
            <div className="w-full px-6 lg:px-12 py-10 flex-1 flex flex-col">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                    <div>
                        ...
                        <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-1">Schedule and monitor institutional class engagements</p>
                    </div>

                    <div className="flex items-center gap-3">
                        <Link 
                            href="/admin/class-sessions/create"
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="border-b border-gray-50 bg-gray-50/30">
                                    ...
                                    <th className="px-6 py-5 text-[11px] font-extrabold text-gray-400 uppercase tracking-wider">Learning Topic</th>
                                    <th className="px-6 py-5 text-[11px] font-extrabold text-gray-400 uppercase tracking-wider text-center">Time Bracket</th>
                                    <th className="px-6 py-5 text-[11px] font-extrabold text-gray-400 uppercase tracking-wider text-center">Delivery</th>
                                    ...
                                    <tr key={session.id} className="group hover:bg-gray-50/50 transition-colors">
                                            ...
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="text-[10px] font-extrabold text-indigo-500 uppercase tracking-wider mt-0.5 flex items-center gap-1">
                                                        <Layers size={10} /> {session.batch?.name}
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-5">
                                            <div className="flex items-center gap-2.5">
                                                ...
                                                <span className="text-sm font-bold text-slate-700 truncate max-w-[200px]">
                                                    {session.topic || 'Subject Matter Pending'}
                                                </span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-5 text-center">
                                            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-100 border border-slate-200 text-slate-600 text-[10px] font-extrabold uppercase tracking-widest">
                                                <Clock size={12} className="text-indigo-400" />
                                                {session.start_time} <ArrowRight size={10} /> {session.end_time}
                                            </div>
                                        </td>
                                        <td className="px-6 py-5">
                                        ...
                                        <td className="px-8 py-5 text-right">
                                            <div className="flex items-center justify-end gap-2">
                                                <Link 
                                                    href={`/admin/class-sessions/${session.id}/edit`}
                                    ...
                                    <tr>
                                        <td colSpan={5} className="px-8 py-20 text-center">
                                            <div className="flex flex-col items-center gap-4">
                                                ...
                                                <div>
                                                    <h3 className="font-bold text-slate-900">No Sessions Scheduled</h3>
                                                    <p className="text-sm text-gray-500 mt-1">Institutional learning calendar is currently empty.</p>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
