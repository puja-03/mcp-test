...
export default function ResultsIndex({ results, exams, filters }: any) {
    ...
    const handleFilter = (e: React.FormEvent) => {
        e.preventDefault();
        router.get('/instructor/results', { exam_id: examId === 'all' ? '' : examId }, { preserveState: true });
    };

    const handleDelete = (id: number) => {
        if (confirm('Are you sure you want to delete this result?')) {
            router.delete(`/instructor/results/${id}`);
        }
    };
        ...
        <>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-6">
                ...
                <form onSubmit={handleFilter} className="flex flex-wrap items-center gap-3">
                    ...
                    {examId !== 'all' && (
                        <Button type="button" variant="ghost" onClick={() => { setExamId('all'); router.get('/instructor/results', {}, { preserveState: true }); }}>
                            Clear
                        </Button>
                    )}
                </form>
                                            ...
                                            <td className="p-4 text-right">
                                                <div className="flex items-center justify-end gap-2">
                                                    <Button variant="ghost" size="icon" asChild>
                                                        <Link href={`/instructor/results/${result.id}/edit`}>
