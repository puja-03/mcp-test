...
export default function Edit({ payment, enrollments }: { payment: Payment, enrollments: Enrollment[] }) {
    const { data, setData, put, processing, errors } = useForm({
        enrollment_id: payment.enrollment_id.toString(),
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/instructor/payments/${payment.id}`);
    };
        ...
        <AppLayout>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold tracking-tight text-foreground">Edit Payment Record</h1>
                        <p className="text-muted-foreground font-medium">Update the transaction details for this payment.</p>
                    </div>
                </div>
                    ...
                    <form onSubmit={submit} className="space-y-8">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="space-y-6">
                                ...
                                <div className="space-y-2">
                                    <Label htmlFor="amount" className="text-xs font-black uppercase tracking-widest text-muted-foreground">Payment Amount</Label>
                            ...
                            <div className="space-y-6">
                                <div className="space-y-2">
                                    ...
                                    <Select defaultValue={data.payment_method} onValueChange={(v) => setData('payment_method', v)}>
                                        <SelectTrigger className="h-12 bg-muted/30 border-muted-foreground/20 focus:ring-primary/40 font-bold">
                        ...
                        <div className="flex gap-4 pt-8">
                            <Button type="submit" disabled={processing} className="h-14 px-12 gap-3 shadow-xl shadow-primary/20 hover:shadow-primary/40 transition-all font-black uppercase tracking-widest text-xs flex-1 md:flex-none">
                                <Save className="h-5 w-5" /> Update Payment Record
                            </Button>
                            <Button variant="ghost" asChild className="h-14 px-8 font-black uppercase tracking-widest text-xs text-muted-foreground hover:bg-muted transition-colors">
                                <Link href="/instructor/payments">Cancel Changes</Link>
                            </Button>
                        </div>
                    </form>
                </div>
