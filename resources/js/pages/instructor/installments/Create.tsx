...
import { ArrowLeft, Save, Calendar as CalendarIcon, DollarSign } from 'lucide-react';
...
export default function Create({ enrollments, feeStructures }: { enrollments: Enrollment[], feeStructures: FeeStructure[] }) {
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post('/instructor/installments');
    };

    return (
        <AppLayout>
            <Head title="Schedule Installment" />
            <div className="flex h-full flex-1 flex-col gap-6 p-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold tracking-tight text-foreground">Schedule Installment</h1>
                        <p className="text-muted-foreground font-medium">Create a dynamic payment schedule for a student.</p>
                    </div>
                </div>
                    ...
                    <form onSubmit={submit} className="space-y-8">
                        <div className="space-y-6">
                            <div className="space-y-2">
                                <Label htmlFor="enrollment_id" className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Select Student / Enrollment</Label>
                                <Select onValueChange={(v) => setData('enrollment_id', v)}>
                                    <SelectTrigger className="h-12 bg-muted/30 border-muted-foreground/20 focus:ring-primary/40 text-sm font-medium">
                                        <SelectValue placeholder="Search student enrollment..." />
                                    </SelectTrigger>
                                    <SelectContent>
                                        {enrollments.map((en) => (
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="fee_structure_id" className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Fee Plan (Optional)</Label>
                                <Select onValueChange={(v) => setData('fee_structure_id', v)}>
                                    <SelectTrigger className="h-12 bg-muted/30 border-muted-foreground/20 focus:ring-primary/40 text-sm font-medium">
                                ...
                                {errors.fee_structure_id && <p className="text-xs text-destructive font-bold mt-1.5">{errors.fee_structure_id}</p>}
                            </div>
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="status" className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Initial Status</Label>
                                <Select defaultValue="pending" onValueChange={(v) => setData('status', v)}>
                        ...
                        <div className="flex gap-4 pt-6 border-t border-dashed">
                            ...
                            <Button variant="ghost" asChild className="h-12 px-8 font-bold uppercase tracking-wider text-xs text-muted-foreground">
                                <Link href="/instructor/installments">Discard Changes</Link>
                            </Button>
                        </div>
                    </form>
                </div>
